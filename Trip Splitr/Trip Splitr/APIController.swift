...
class APIController {
    ...
    func register(with name: String, username: String, password: String, email: String, completion: @escaping (Error?) -> Void) {
        ...
        let user = Register(name: name, username: username, password: password, email: email)

        do {
            request.httpBody = try JSONEncoder().encode(user)
        } catch {
            NSLog("Error encoding register: \(error)")
            completion(error)
            return
        }
    ...
    func getTrip(tripID: Int, completion: @escaping (Result<Trip, NetworkError>) -> Void) {
        ...
        let requestURL = baseURL
            .appendingPathComponent("trips/\(tripID)")
        ...
        URLSession.shared.dataTask(with: request) { (data, _, error) in
            ...
            do {
                ...
                NSLog("Error decoding trip: \(error)")
